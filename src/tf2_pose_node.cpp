...
{
  try {
    // 1. Get the latest available transform
    auto transform = tf_buffer_->lookupTransform(
      output_frame_, 
      target_frame_, 
      tf2::TimePointZero);

    // 2. Check if the transform is stale (i.e. the TF publisher stopped)
    rclcpp::Time tf_time(transform.header.stamp);
    rclcpp::Duration age = this->now() - tf_time;

    if (age.seconds() > timeout_s_) {
      RCLCPP_DEBUG_THROTTLE(this->get_logger(), *this->get_clock(), 5000, 
                           "TF is stale (age: %.2fs). Waiting for fresh data...", age.seconds());
      return; // Exit the callback without publishing
    }

    // 3. Build and publish the pose
    ...
    // Only print warnings if the node has been running for more than 3 seconds
    if (this->now().nanoseconds() > 3e9) { 
